...
def run_hmmsearch(hmm, seqs, has_gathering=False):
    hmm_results = {}
...
print(f"\nrunning for {prefix}")
with open(f"hmmsearch_outputs_5kb/{prefix}.tsv", "a") as fout:
    ...
    for hmm in hmm_list:
        ...
        if "PFAM" in hmm.parts[-1] or "NCBIFAM" in hmm.parts[-1]:
            hmmsearch = run_hmmsearch(hmm, seqs, has_gathering=True)
        else:
            hmmsearch = run_hmmsearch(hmm, seqs)
        
        print("Writing results")
        for hit_list in hmmsearch.values():
