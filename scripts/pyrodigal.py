...
def process(sequence):
     return (sequence.accession, orf_finder.find_genes(sequence.seq))

def main():
    with multiprocessing.pool.Pool() as pool, open("test.faa", "w") as fout:
        for seq_id, preds in pool.imap(process, read_fasta("test.fna")):
            for i, pred in enumerate(preds):
                fout.write(f">{seq_id}_{i}\n")
                fout.write(f"{pred.translate()}\n")
